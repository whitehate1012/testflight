...
export default function Sites() {
  ...
  const filteredSites = useMemo(() => {
    ...
    if (!q) return sites;
    return sites.filter((s) => [s.name, s.location, s.inchargeName].some((v) => v?.toLowerCase().includes(q)));
  }, [search, sites]);
  ...
  const toggleRow = (siteId: string) => setExpanded((p) => ({ ...p, [siteId]: !p[siteId] }));
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Tap a site to expand and view foremen; click a foreman to view attendance</CardDescription>
        </CardHeader>
        <CardContent>
                              ...
                              <TableCell colSpan={4} className="bg-gray-50">
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25, ease: "easeInOut" }}>
                                  <div className="p-4 border rounded-md">
                                    <div className="text-sm font-medium mb-2">Site Foremen</div>
                                      ...
                                      {siteForemen.length === 0 ? (
                                        ...
                                        siteForemen.map((f) => (
                                          <div key={f.id} className="flex items-center justify-between border rounded px-2 py-1.5 text-sm bg-white">
                                            ...
                                            <Button size="sm" variant="outline" className="h-7 px-2" onClick={() => openForemanDialog(f.id)}>
                                              View
                                            </Button>
                                          </div>
                                        ))
